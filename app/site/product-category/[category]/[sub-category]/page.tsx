...
export default function ProductListing({ params }: { params: { subCategory: string } }) {
  const category = categories[params.subCategory as keyof typeof categories]
      ...
      <div className="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8 py-16">
        ...
        <div className="grid grid-cols-1 gap-y-10 gap-x-6 sm:grid-cols-2 lg:grid-cols-3">
          {category.subCategories.map((subCategory) => (
            <Link
              key={subCategory.id}
              href={`/site/product-category/${params?.subCategory}/${subCategory.id}`}
              className="group"
            >
